...
# Dynamic Hand Gestures Classification: FPHA Dataset
...
def _set_view(v_orientation: str, camera_center=None, camera_scale=None):
    ...
    if v_orientation == "top-down":  # TOP--> LOOKING DOWN
        ...
        view.camera.elevation, view.camera.azimuth = 0.0, 180.0
        ...
        view.camera.scale_factor = 1000 if cfg.s_fitting == "min-max" else 600
        view.camera.elevation, view.camera.azimuth = 25.0, 115.0  # -25.0, -75.0
...
def _create_sequence_png(dir):
    ...
    if cfg.dataset_create_mode:
        i_subset, i_gesture = fname.split("-")
        dir = dir.joinpath(f"{i_subset}/{i_gesture}/gs{seq_idx+stt_lim:04}")
        prefix = ""
...
class loadConfigArguments:
    ...
    def init_gesture_sequences_and_labels(self, data):
        if self.dataset_subset == "train":
            ...
            str_labels = "train-" + np.array(data["train_labels"], dtype=object)

        elif self.dataset_subset == "valid":
            self.ds_g_sequences = data["X_valid"]
            str_labels = "valid-" + np.array(data["valid_labels"], dtype=object)
            ...
            str_labels = np.concatenate(
                (
                    "train-" + np.array(data["train_labels"], dtype=object),
                    "valid-" + np.array(data["valid_labels"], dtype=object),
                )
            )
...
ap.add_argument(
    ...
    "--config",
    required=False,
    default="modules/.configs/fpha-v5-default.hgr-config",
