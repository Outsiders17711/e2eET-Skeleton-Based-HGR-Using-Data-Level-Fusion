# Dynamic Hand Gestures Classification: DHG 14/28 Dataset
...
def _set_view(v_orientation: str, camera_center=None, camera_scale=None):
    ...
    if v_orientation == "top-down":  # TOP--> LOOKING DOWN
        ...
        view.camera.scale_factor = 1000 if cfg.s_fitting == "min-max" else 0.5
...
def update(null):
    ...
    if not cfg.dataset_create_mode:
        info.text = f"i{seq_idx+stt_lim}.g{int_labels[seq_idx]}.{str_labels[seq_idx]}\n[{tmp_v_orientation}] views\n{cfg.s_fitting} fitting"
...
def _create_sequence_png(dir):
    ...
    prefix = f"{fname.split('-')[-1]}-"

    if cfg.dataset_create_mode:
        i_subset, i_gesture, i_id = fname.split("-")
        dir = dir.joinpath(f"{i_subset}/{int_labels[seq_idx]:02}-{i_gesture}/{i_id}")
        prefix = ""
...
def get_camera_details():
    print(
        f">>> @i{seq_idx+stt_lim}.g{int_labels[seq_idx]}:",
...
class loadConfigArguments:
    def __init__(self, args) -> None:
        ...
        self.imgs_ds_dir = Path(
            ...
            .replace("<NG>", f"{self.n_dataset_classes}g")
    ...
    def init_gesture_sequences_and_labels(self, data):
        if self.dataset_subset == "train":
            ...
            int_labels = np.array(data[f"y_train_{self.n_dataset_classes}"], dtype=int)
            str_labels = "train-" + np.array(data["train_str_labels"], dtype=object)

        elif self.dataset_subset == "valid":
            self.ds_g_sequences = data["X_valid"]
            int_labels = np.array(data[f"y_valid_{self.n_dataset_classes}"], dtype=int)
            str_labels = "valid-" + np.array(data["valid_str_labels"], dtype=object)

        else:  # elif self.dataset_subset == "all":
            self.ds_g_sequences = np.concatenate((data["X_train"], data["X_valid"]), axis=0)
            int_labels = np.array(
                data[f"y_train_{self.n_dataset_classes}"] + data[f"y_valid_{self.n_dataset_classes}"],
                dtype=int,
            )
            str_labels = np.concatenate(
                (
                    "train-" + np.array(data["train_str_labels"], dtype=object),
                    "valid-" + np.array(data["valid_str_labels"], dtype=object),
                )
            )

        self.ds_g_sequences = np.abs(self.ds_g_sequences)
        ...
        return l_dataset, n_skeletons, int_labels, str_labels
    ...
    def do_sequence_fitting(self):
        ...
        return self.ds_g_sequences, abs_means, means, diffs
...
def main(stt_end_args):
    global ds_g_sequences, int_labels, str_labels, means, diffs, stt_lim, canvas
    (stt_lim, end_lim), n = stt_end_args

    ds_g_sequences = ds_g_sequences[stt_lim:end_lim]
    int_labels, str_labels = int_labels[stt_lim:end_lim], str_labels[stt_lim:end_lim]
...
ap.add_argument(
    ...
    "--config",
    required=False,
    default="modules/.configs/dhg1428-v5-default.hgr-config",
...
l_dataset, n_skeletons, int_labels, str_labels = cfg.init_gesture_sequences_and_labels(data)
...
# [>>>>>]____________________________________________________________
# python modules/create_imgs_v5_DHG1428d_mVOs_ssPVs.py -c "modules/.configs/dhg1428-v5-default.hgr-config"
